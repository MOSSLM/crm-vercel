...
/**
 * Appliquer une rédaction d'audit — le cœur, hors de toute route.
 *
 * POURQUOI CE MODULE EXISTE. Ce code vivait dans `POST /api/audit/preparation`,
 * derrière `requireUser`. La préparation d'un dossier depuis un poste local — la
 * boucle qualifier → enrichir → auditer — devait donc soit ouvrir une session,
 * soit recopier la logique dans un script. La recopier, c'était accepter qu'elle
 * diverge au premier changement de contrat : deux barèmes, deux vérités, et
 * personne pour s'apercevoir laquelle est partie chez le prospect.
 *
 * La route garde ce qui la concerne — l'authentification, le corps JSON, les
 * codes HTTP — et appelle ceci. Le script d'atelier appelle la même fonction
 * avec le client de service. Il n'y a qu'une implémentation du contrat.
 */
...
export interface ResultatPreparation {
  ...
  lien_rouvert?: boolean;
  rejets: unknown[];
  observations: unknown[];
  erreur?: { message: string; statut: number };
}

export async function appliquerPreparation(
  sb: SupabaseClient,
  params: { opportuniteId: string; entrepriseId: number; preparation: unknown; userId?: string | null },
): Promise<ResultatPreparation> {
  const { opportuniteId, entrepriseId, preparation } = params;

  const dossier = await construireDossier(sb, entrepriseId);
  if (!dossier) return { applique: false, rejets: [], observations: [], erreur: { message: "Entreprise introuvable.", statut: 404 } };

  const verdict = validerPreparation(preparation, universDe(dossier));
  ...
  if (error) return { applique: false, rejets: verdict.rejets, observations: verdict.observations, erreur: { message: `Écriture impossible : ${error.message}`, statut: 500 } };
   ...
   */
  const { erreur: erreurJeton } = await activerJetonRapport(sb, entrepriseId, params.userId ?? null);

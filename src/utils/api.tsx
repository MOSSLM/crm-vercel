...
export const companiesApi = {
    ...
    try {
      ...
      while (true) {
        ...
        if (validCompanies.length > 0) {
          const sanitizedCompanies = validCompanies.map((company) =>
            buildCompanyFromPartial(company.id, company)
          );
          allCompanies.push(...sanitizedCompanies);
        }
    ...
    try {
      while (true) {
        ...
        if (validCompanies.length > 0) {
          ...
          allQualified.push(...sanitizedCompanies);
        }
  ...
  create: async (
    ...
    try {
      ...
      if (isCompanyRow(data)) {
        return buildCompanyFromPartial(data.id, data);
      }
  ...
  getById: async (id: number) => {
    try {
      ...
      return buildCompanyFromPartial(company.id, company);
    } catch (error) {

        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          {p.solutions.map((sol, i) => (
            <Zone key={i} field={`page3.solutions.${i}`} activeField={activeField} onFieldClick={onFieldClick}>
              <div style={{ display: 'grid', gridTemplateColumns: '32px 1fr auto', alignItems: 'start', gap: 16, padding: '18px 0', borderBottom: i < p.solutions.length - 1 ? `1px solid rgba(10, 27, 51,0.07)` : 'none' }}>
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 28, fontWeight: 300, color: 'rgba(47, 122, 224,0.25)', lineHeight: 1 }}>{sol.num}</div>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 500, color: C.nuit, marginBottom: 4 }}>{sol.name}</div>
                  <div style={{ fontSize: 11, lineHeight: 1.6, color: 'rgba(10, 27, 51,0.55)' }}>{sol.desc}</div>
                </div>
                <div style={{ fontSize: 9, letterSpacing: '0.12em', textTransform: 'uppercase', color: C.azur, background: 'rgba(47, 122, 224,0.08)', padding: '4px 8px', borderRadius: 2, whiteSpace: 'nowrap', fontWeight: 500 }}>{sol.tag}</div>
              </div>
            </Zone>
          ))}
        </div>

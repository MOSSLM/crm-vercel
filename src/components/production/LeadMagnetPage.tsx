...
export const LeadMagnetPage: React.FC = () => {
  const getPublicEndpointUrl = (row: Record<string, unknown>): string | null => {
    ...
    return `${origin}/api/public/lead-magnets/${encodeURIComponent(String(id).trim())}`;
  };

  return (
    <ProductionDataTable
      title="Lead magnet"
      description="Consultez/modifiez les contenus et copiez l'endpoint public (basé sur l'ID) pour Framer."

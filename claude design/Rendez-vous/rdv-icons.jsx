// rdv-icons.jsx — jeu d'icônes du module Rendez-vous (style lucide, 24px)
const __RV_ICONS = {
...
function Icon({ name, className = "ico", strokeWidth, ...rest }) {
  const p = __RV_ICONS[name];
  if (!p) return <span style={{ display: "inline-block", width: 14, height: 14 }} />;
  return <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={strokeWidth || 1.6} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" {...rest}>{p}</svg>;
}
window.Icon = Icon;
